...
$\Delta\mathbf g=\mathbf g_k-\mathbf g_{k-1}$, each step corrects $B$ by a Broyden
secant assimilation~\cite{broyden1965,nocedal2006} and relaxes it slowly toward the
identity,
\begin{equation}
  B \;\leftarrow\; B
    +\frac{(\Delta\mathbf g-B\,\Delta\mathbf p)\,\Delta\mathbf p^{\mathsf T}}
          {\Delta\mathbf p^{\mathsf T}\Delta\mathbf p}
    +\lambda\,(\mathbb 1-B)\,\Delta t,
  \qquad B\leftarrow\tfrac12\bigl(B+B^{\mathsf T}\bigr),
  \label{eq:broyden}
\end{equation}
applied only where $\lVert\Delta\mathbf p\rVert$ exceeds a small gate (otherwise $B$
is held), the symmetrization imposing the symmetry of a Hessian. The two added terms
...
$\Delta t$ (Broyden is a discrete iteration, with no $\Delta t\to0$ limit). The second
...
toward $\mathbb 1$ supplies a safe default ($B_{yy}=1$) where the step is too small to
measure. Equivalently, $B$ follows a continuous leak $\dot B=-\lambda(B-\mathbb 1)$
...
\begin{figure*}[!t]
  ...
  \caption{%
    ...
    inversion gain $s$: red ($s\to+1$) on the approach to the collinear saddle
    $L_1$, blue ($s\to-1$) on the approach to the triangular minimum $L_4$.
...
$B=\mathbb 1$, $s=-1$ (the safe descending state), and the Broyden
update~\eqref{eq:broyden} is applied only when $\lVert\Delta\mathbf p\rVert$ exceeds
a small gate, so that near a fixed point---where $\Delta\mathbf p\to0$---the estimate
...
$\lambda$ & Broyden leak toward $\mathbb 1$ & $2.0$\\
