...
\emph{quasi-Newton} estimate of the Hessian from successive gradient samples, by the
Broyden update~\cite{broyden1965,nocedal2006}: with
$\Delta\mathbf p=\mathbf p_k-\mathbf p_{k-1}$ and
$\Delta\mathbf g=\mathbf g_k-\mathbf g_{k-1}$,
\begin{equation}
  B \;\leftarrow\; B+\frac{(\Delta\mathbf g-B\,\Delta\mathbf p)\,
       \Delta\mathbf p^{\mathsf T}}{\Delta\mathbf p^{\mathsf T}\Delta\mathbf p},
  \qquad B\leftarrow\tfrac12\bigl(B+B^{\mathsf T}\bigr),
  \label{eq:broyden}
\end{equation}
the symmetrization imposing the symmetry of a Hessian. Because the curvature varies
across the landscape, $B$ is given a slow leak toward the identity so that it tracks
the \emph{local} value,
\begin{equation}
  B \;\leftarrow\; B+\lambda\,(\mathbb 1-B)\,\Delta t .
  \label{eq:leak}
\end{equation}
...
\eqref{eq:broyden}--\eqref{eq:leak} and $s$ by \eqref{eq:sdot} at each step. No
